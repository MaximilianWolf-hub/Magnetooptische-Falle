...
### 1. TIFF-Datei laden ###
image = tiff.imread("Test.tiff")  # Bild laden
...
p0 = [np.max(image), nx//2, ny//2, nx/4, ny/4, np.min(image)]  # Schätzung für Parameter

### 4. Curve Fit ausführen ###
params, cov = opt.curve_fit(gaussian_2D, (xdata, ydata), zdata, p0=p0)

# Gefittete Parameter extrahieren
A_fit, x0_fit, y0_fit, sigma_x_fit, sigma_y_fit, C_fit = params
print(f"Gefittete Parameter:\n A={A_fit}, x0={x0_fit}, y0={y0_fit}, sigma_x={sigma_x_fit}, sigma_y={sigma_y_fit}, C={C_fit}")
